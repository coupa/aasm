class AASMLegacy::SupportingClasses::Event
  ...
  def fire(obj, to_state=nil, *args)
    ...
    raise AASMLegacy::InvalidTransition, "Event '#{name}' cannot transition from '#{obj.aasm_current_state}'" if transitions.size == 0
  ...
  def transitions(trans_opts)
    Array(trans_opts[:from]).each do |s|
      @transitions << AASMLegacy::SupportingClasses::StateTransition.new(trans_opts.merge({:from => s.to_sym}))
    end
  end
end
